...
test.describe('Step Content Validation', () => {
  ...
  test('Step 1: Listening for Connections - content check', async ({ page }) => {
    ...
    await expect(page.getByText(/Create a TCP server/i)).toBeVisible();
    await expect(page.getByText(/port 6379/i)).toBeVisible();

    // Check for code hints
    await expect(page.getByText(/socket.bind/i)).toBeVisible();
  });
  ...
  test('Step 2: Handle PING Command - content check', async ({ page }) => {
    ...
    // Check for key concepts
    await expect(page.getByText(/PING command/i)).toBeVisible();
    await expect(page.getByText(/PONG/i)).toBeVisible();
    await expect(page.getByText(/RESP/i)).toBeVisible();
  });
  ...
  test('Step 3: Handle ECHO Command - content check', async ({ page }) => {
    ...
    await expect(page.getByText(/RESP Arrays/i)).toBeVisible();
    await expect(page.getByText(/Bulk String/i)).toBeVisible();
  });
  ...
  test('Step 4: Handle SET Command - content check', async ({ page }) => {
    ...
    await expect(page.getByText(/In-Memory Storage/i)).toBeVisible();
    await expect(page.getByText(/key-value pairs/i)).toBeVisible();
  });
  ...
  test('Step 5: Handle GET Command - content check', async ({ page }) => {
    ...
    await expect(page.getByText(/GET command/i)).toBeVisible();
    await expect(page.getByText(/retrieve/i)).toBeVisible();
  });
